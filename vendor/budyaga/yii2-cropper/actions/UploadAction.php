            ...
            if ($model->hasErrors()) {
                ...
                switch($this->imageType) {
                    case 'profile':
                        ...
                        if(!file_exists(Yii::getAlias('@frontend').'/web/uploads/user/photo/original_images/'.$id)) {
                            mkdir(Yii::getAlias('@frontend').'/web/uploads/user/photo/original_images/'.$id,0775, true);
                        }
                        $userModel = \backend\models\User::findOne($id);
                        $encodedEmailUniqueId = substr(base64_encode($userModel->email),0,8);                        
                        $model->{$this->uploadParam}->name = $encodedEmailUniqueId . '.jpg';
